...
class DataWrapper(object):
    ...
    def reinit_output_columns(self):
        _raw_production_values: pd.DataFrame = self._output[DEF_PRODUCTION_SHEET]
        ...
        _raw_emission_values: pd.DataFrame = self._output[DEF_EMISSION_SHEET]
        _raw_emission_values.columns = DEF_EMISSION_COLUMNS
        self.emission_values = _raw_emission_values

        _raw_capacity_values: pd.DataFrame = self._output[DEF_CAPACITY_SHEET]
        ...
        _raw_cost_values: pd.DataFrame = self._output[DEF_EXOGENOUS_COST_SHEET]
        _raw_cost_values.columns = DEF_EXOGENOUS_COST_COLUMNS
        self.cost_values = _raw_cost_values

        _raw_detailed_cost_values: pd.DataFrame = self._output[DEF_DETAILED_COST_SHEET]
        ...
        _raw_trade_capacity_values: pd.DataFrame = self._output[DEF_TRADE_CAPACITY_SHEET]
